...
def shell():
    ...
    if args.download:
        ...
        printer(((results.download / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
	printer('testing sameer')
	DownloadSpeed=results.download / 1000.0 / 1000.0
	printer(DownloadSpeed)
    else:
    ...
    if args.upload:
        ...
        printer('Upload: %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
	printer('testing sameer upload')
	UploadSpeed=results.upload / 1000.0 / 1000.0
	printer(UploadSpeed)
	rc = subprocess.call("/home/Aggregator/SaveInternetSpeed.sh " + str(DownloadSpeed)+" "+str(UploadSpeed), shell=True)
